...
const UPDATE_BASE_URL = 'https://github.com/jeanbmar/black-moon-rewind/releases/download';

const resourcesPath = process.env.RESOURCES_PATH ?? process.resourcesPath;
const gameVersion = process.env.BMC_GAME_VERSION ?? '1.0.0';
const gameExePath = process.env.BMC_GAME_EXE_PATH ?? path.resolve(resourcesPath, 'game', gameVersion, 'game.exe');
const gameRootDir = path.join(resourcesPath, 'game');
const gameTmpDir = path.join(gameRootDir, `black-moon-rewind-game-${gameVersion}`);
